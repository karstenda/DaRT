...
public class Matrix {
	...
	public void removeHomogeneousCoordinates() {
		for (int i = 0; i < nCols; i++) {
			...
			double denominator = data[i][data[i].length-1];
			for (int j = 0; j < data[i].length-1; j++) {
				newCol[j] = data[i][j]/denominator;
			}
	...
	public static void debug(Vector vector) {
	...
	public static void debug(Matrix matrix) {
		...
		System.out.println("debug columns are: "+matrix.getData().length);
		System.out.println("debug rows are: "+matrix.getData()[0].length);
		
		for (int i=0; i <matrix.getData()[0].length;i++) {
